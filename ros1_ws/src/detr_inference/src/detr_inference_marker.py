...
class DetrInferenceNode:
    def __init__(self):
        ...
        self.last_marker_count = 0  # Initialize last marker count for deletion
    ...
    def init_publishers(self):
        ...
        self.pub_detection_image = rospy.Publisher(
            rospy.get_param('~pub_camera_topic', '/detection_result_img/camera_stitched/compressed'),
            CompressedImage, queue_size=1
        )
        self.pub_marker_array = rospy.Publisher(
            rospy.get_param('~pub_marker_topic', '/detr/camera_mid/boxes'),
    ...
    def detection_callback(self, msg):
        ...
        if pil_image:
            ...
            if len(detections["scores"]) > 0:
                ...
                # Publish detections
                self.delete_markers()  # Clear previous markers
                self.publish_markers(scores, labels, boxes, msg_header)
    ...
    def publish_markers(self, scores, labels, boxes, header):
    ...
    def delete_markers(self):
        """Delete all markers by publishing an empty MarkerArray."""
        marker_array = MarkerArray()
